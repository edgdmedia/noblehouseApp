...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var pwaName = "noble"; //Local Storage Names for PWA
    ...
    function init_template() {
        ...
        function formatDate(dateString, options) {
            ...
            // Return full date in text
            if (options.fullDate) {
                return `${date.toLocaleDateString('en-US', { weekday: 'long' })}, ${date.toLocaleDateString('en-US', { day: 'numeric', month: 'long' })}, ${date.getFullYear()} at ${date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
            }

            // Default case: relative time
            if (currentDate.toDateString() === date.toDateString()) {
                const diffMinutes = Math.floor(diffTime / (1000 * 60));
                return diffMinutes < 60 ? `${diffMinutes} minutes ago` : `${Math.floor(diffMinutes / 60)} hours ago`;
            } else if (diffDays === 1) {
        ...
        setStorage("queryAlbums", {
            ...
            orderBy: 'id',
            status: "",
            fetchUrl: 'https://noblehousecc.org/wp-json/wp/v2/album?_embed'
        }, true);

        setStorage("queryTracks", {
            dataType: 'Tracks',
            div: "",
            totalPages: "",
            totalResults: "",
            ...
            parent: "",
            media_type: 'audio',
            fetchUrl: 'https://noblehousecc.org/wp-json/wp/v2/media?'
        }, true);
        ...
        if (dataDivs.length) {
            ...
            dataDivs.forEach(function (el) {
                ...
                if (dataType === "albums") {
                    ...
                    let currentAlbum = getStorage('album-current');
                    $("#album-image").html(
                        `<img src="${currentAlbum.imageUrl}" data-src="${currentAlbum.imageUrl}" class=" preload-img card mb-0 img-fluid rounded-m entered loaded" alt="img" data-ll-status="loaded" loading="lazy" />`
                    );
                    $("#album-title").html(currentAlbum.title);
                    $("#album-minister").html("Rev. Bayode Oyekan");
                    let queryData = getStorage('queryTracks');
                    queryData.parent = currentAlbum.id;
                    queryData.div = el;
                    fetchTracks(queryData);
                }
            });
            $('#first-page').on('click', function () {
                let currentPage = 1;
                let storageData = getStorage('queryAlbums')
            ...
            $('#next-page').on('click', function () {
                let storageData = getStorage('queryAlbums')
                if (storageData.currentPage === (Math.floor(storageData.totalPages - 1))) {
                    storageData.currentPage = storageData.totalPages;
                } else {
                    storageData.currentPage = Math.floor(storageData.currentPage + 1);
                    console.log(storageData.currentPage);
                }
                setStorage('queryAlbums', storageData);
                fetchData(storageData);
            });
        ...
        //     return $('.data-div[data-type="' + dataType + '"]');
        // }

        function getCurrentDataType() {
            let dataType = getStorage("dataType")
        ...
        function fetchData(queryData) {
            ...
            $.get(queryData.fetchUrl, {
                ...
                .done(function (data, textStatus, jqXHR) {
                    var totalResults = jqXHR.getResponseHeader('X-WP-Total');
                    var totalPages = jqXHR.getResponseHeader('X-WP-TotalPages');
                    queryData.totalPages = parseInt(totalPages);
                    queryData.totalResults = parseInt(totalResults);
                    setStorage('query' + queryData.dataType, queryData);
                    setStorage("data" + queryData.dataType, data);
                    updateDataDiv(data, queryData);
                    renderPagination(queryData.dataType);
                })
        ...
        function fetchTracks(queryData) {
            $.get(queryData.fetchUrl, {
                search: queryData.searchTerm,
                orderby: queryData.orderBy,
                per_page: queryData.perPage,
                ...
                media_type: queryData.media_type,
            })
                .done(function (data, textStatus, jqXHR) {
                    var totalResults = jqXHR.getResponseHeader('X-WP-Total');
                    var totalPages = jqXHR.getResponseHeader('X-WP-TotalPages');
                    queryData.totalPages = parseInt(totalPages);
                    queryData.totalResults = parseInt(totalResults);
                    setStorage('queryTracks', queryData);
                    setStorage('dataTracks', data);
                    updateDataDiv(data, queryData);
                })
                .fail(function (jqXHR, textStatus, errorThrown) {
                    if (jqXHR.status === 404) {
        ...
        function updateDataDiv(data, queryData) {
                ...
                $.each(data, function (index, item) {
                    ...
                    var album = `<a class="" href="#">
                                  <div class="card card-style " loading="lazy" style="background-image: url(${albumImageUrl}); min-height:350px; -webkit-transform: none !important;">
                                    <div class="">
                                      <div class=" m-auto card-center rounded-l bg-fade-gray-light p-3" style="width:max-content;">
                                          <div class="d-flex justify-content-between align-items-center">
                                              <div class="icon icon-s bg-theme rounded-l shadow-l rounded-m  color-theme" >
                                              <i class="fa fa-play font-14"></i>
                                              </div>
                                          </div>
                                      </div>
                                      </div>
                    ...
                    // Convert noteType string to jQuery object
                    let $album = $(album);
                    // Add onclick listener to the noteType element
                    $album.click(function () {
                        setStorage("album-current", {
                            id: item.id,
            ...
            } else if (queryData.dataType === 'Tracks') {
                let currentAlbum = getStorage('album-current');
                $("#album-desc").html(queryData.totalPages + (queryData.totalPages.length > 1 ? 'Tracks' : 'Track') + ' - ' + currentAlbum.date);
                $.each(data, function (index, item) {
                    console.log(item);
                    var track = ` 
                    <div class="d-flex align-items-center p-2">
                        <h3 class="p-2 m-0 fw-bold">${(index + 1).toString().padStart(2, '0')}</h3>
                        <div class="p-2 w-100 ">
                            <h4 class="m-0">${item.title.rendered}</h4>
                            <div class="text-opacity-75">${formatDate(item.date, { date: true })} . ${item.media_details.length_formatted}</div>
                        </div>
                        <div class="dropdown">
                            <a class=" dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="fa text-opacity-50 fa-ellipsis font-20 "></i></a>
                            <ul class="dropdown-menu rounded border-0 shadow">
                                <li><i class="fa dropdown-item text-opacity-50 fa-download font-20 "> </i>Download</li>
                                <li><i class="fa dropdown-item text-opacity-50 fa-cloud-download-alt font-20 "></i>Save</li>
                                <li><i class="fa dropdown-item text-opacity-50 fa-video font-20 "></i>Watch</li>
                                <li><i class="fa dropdown-item text-opacity-50 fa-music font-20 "></i>Spotify</li>
                            </ul>
                        </div>
                    </div>`;
                    // Convert noteType string to jQuery object
                    ...
                    $track.click(function () {
                        ...
                        if ('mediaSession' in navigator) {
                            navigator.mediaSession.metadata = new MediaMetadata({
                                title: item.title.rendered,
                                artist: item.media_details.artist,
                                album: item.media_details.album, // Optionally set an album name
                                artwork: [
                                    { src: currentAlbum.imageUrl, sizes: '512x512', type: 'image/jpg' }
                                ]
                            });

                            // Optional: You can also handle media controls such as play, pause, seek, etc.
                            ...
                            navigator.mediaSession.setActionHandler('seekforward', function () {
                                $('#audio-player')[0].currentTime = Math.min($('#audio-player')[0].duration, $('#audio-player')[0].currentTime + 10); // Fast forward 10 seconds
                            });

                            // Handle the seek (when user drags the seeker)
                            navigator.mediaSession.setActionHandler('seekto', function (details) {
                                if (details.fastSeek && 'fastSeek' in $('#audio-player')[0]) {
                                    $('#audio-player')[0].fastSeek(details.seekTime);
                                } else {
                                    $('#audio-player')[0].currentTime = details.seekTime;
                                }
                            });
                            // Optional: Handle stop action (if your app requires it)
                            navigator.mediaSession.setActionHandler('stop', function () {
                                $('#audio-player')[0].pause();
                                $('#audio-player')[0].currentTime = 0;
                            });
                        }
                        setStorage("track-current", {
                            id: item.id,
                            url: item.source_url,
                            title: item.title['rendered'],
                    ...
                    $(dataDiv).append($track);
                })
            }
        }

        function renderPagination(dataType) {
            let storageData = getStorage("query" + dataType);
            let totalPages = storageData.totalPages;
            let currentPage = storageData.currentPage;
            ...
            if (totalPages <= 3) {
                startPage = 1;
                endPage = totalPages;
                $('#first-page, #previous-page, #last-page, #next-page').hide();
            } else {
                if (currentPage === 1) {
                    startPage = 1;
                    endPage = 3;
                    $('#first-page, #previous-page').hide();
                    $('#last-page, #next-page').show();
                } else if (currentPage === totalPages) {
                    startPage = totalPages - 2;
                    endPage = totalPages;
                    $('#next-page, #last-page').hide();
                    $('#previous-page, #first-page').show();
                } else {
                    startPage = currentPage - 1;
                    endPage = currentPage + 1;
                    $('#first-page, #previous-page, #last-page, #next-page').show();
                }
            }

            for (var i = startPage; i <= endPage; i++) {
                var paginationItem = $('<a>').text(i).addClass('page-item page-link py-2 rounded-xs color-black bg-theme shadow-xl border-0');
                paginationItem.attr('href', '#');
                paginationItem.attr('data-page', i);

                if (i === parseInt(currentPage)) {
                    paginationItem.addClass('active color-white bg-highlight ');
                    paginationItem.removeClass('color-black bg-theme');
                }
                ...
                paginationItem.on('click', function (event) {
                    ...
                    storageData.currentPage = currentPage;
                    setStorage("query" + dataType, storageData);
                    fetchData(storageData);
                });
        ...
        // var albumDiv = document.querySelectorAll(".album-item");
        // if (albumDiv.length) {
        //     let albumId = getStorage('album-current');
        //     if (albumId) {

        //         // Open or create the database
        //         const request = indexedDB.open('NoblehouseDB', 1);
        ...
        //                         albumDiv.forEach(function (el) {
        //                             let albumDivItem = el.getAttribute("data-item");
        //                             if (albumDivItem === 'minister') {
        //                                 $(el).text('Rev. Bayode Oyekan')
        //                             } else if (albumDivItem === 'title') {
        //                                 $(el).text(album.title)
        //                             } else if (albumDivItem === 'image') {
        //                                 if (album.image) {
        //                                     var mediaUrl = album.image;
        //                                 } else {
        //                                     mediaUrl = "/"
        //                                 }
        //                                 $(el).html(`<img
        //                                 src="${mediaUrl}"
        //                                 data-src="${mediaUrl}"
        //                                 class=" preload-img card mb-0 img-fluid rounded-m entered loaded"
        //                                 alt="img"
        ...
        //                             }
        //                         })
        //                         // Do something with the fetched album
        //                     } else {
        ...
        function livestream() {
            const url = "https://noblehousecc.org/wp-json/wp/v2/settings";

            // Your WordPress username and application password
            const username = 'nadmin';
            const appPassword = 'Na1s PVvM nOW4 ilCb kS3L Tk7A';
            ...
            $.ajax({
                url: 'https://noblehousecc.org/wp-json/wp/v2/settings',
                method: 'GET',
                headers: {
                ...
                success: function (data) {
                    ...
                    $('#live-yt').html(`<a id="live-fb" class="embedly-card" data-card-controls="0"
                            href="${data.youtube}"></a>`);
                    $('#live-fb').html(`<a id="live-fb" class="embedly-card" data-card-controls="0"
                            href="${data.facebook}"></a>`);
                },
                error: function (xhr, status, error) {
                    console.log(error, status);
                    // Handle any errors
                    // $('#settingsContainer').html('<p>Error fetching settings: ' + error + '</p>');
                }
            });
        }
